...
var spanToUse = document.getElementsByClassName("help-toggle toggle");

var sampleCrits = document.createElement('div');
sampleCrits.setAttribute("class", "option active sample_crits ca_tooltip");
sampleCrits.innerHTML = 'sample critiques';


var caTooltipText = document.createElement('div');
caTooltipText.setAttribute("class", "ca_tooltip_text");


var header = document.createElement('div');
header.setAttribute("id", "ca_header");
var title = document.createElement('h3');
title.setAttribute("id", "ca_title");
var crit = document.createElement('span');
var aid = document.createElement('span');
crit.innerHTML = 'Critique';
crit.setAttribute("id", "ca_yellow");

aid.innerHTML = 'Aid';
aid.setAttribute("id", "ca_blue");

title.appendChild(crit);
title.appendChild(aid);
...
var crit_col1 = document.createElement('div');
var crit_col2 = document.createElement('div');
crit_col1.setAttribute("class", "ca_crit_cols");
crit_col1.setAttribute("id", "crit_col1");
crit_col2.setAttribute("class", "ca_crit_cols");
crit_col2.setAttribute("id", "crit_col2");

var col1_head = document.createElement('p');
col1_head.setAttribute("class", "crit_col_heads");
col1_head.setAttribute("id", "crit_col1_head");
var col1_bod = document.createElement('p');
col1_bod.setAttribute("class", "crit_col_bods");
col1_bod.setAttribute("id", "crit_col1_data");

var col2_head = document.createElement('p');
col2_head.setAttribute("class", "crit_col_heads");
col2_head.setAttribute("id", "crit_col2_head");
var col2_bod = document.createElement('p');
col2_bod.setAttribute("class", "crit_col_bods");
col2_bod.setAttribute("id", "crit_col2_data");

var col1_foot = document.createElement('a');
col1_foot.innerHTML = 'Click to view the original post for more context.';
col1_foot.setAttribute("id", "ca_crit1_link");
col1_foot.setAttribute("target", "_blank");
var col2_foot = document.createElement('a');
col2_foot.innerHTML = 'Click to view the original post for more context.';
col2_foot.setAttribute("id", "ca_crit2_link");
col2_foot.setAttribute("target", "_blank");

crit_col1.appendChild(col1_head);
crit_col1.appendChild(col1_bod);
crit_col1.appendChild(col1_foot);

crit_col2.appendChild(col2_head);
crit_col2.appendChild(col2_bod);
crit_col2.appendChild(col2_foot);

crit_row.appendChild(crit_col1);
crit_row.appendChild(crit_col2);

body.appendChild(result);
body.appendChild(crit_row);

var footer = document.createElement('div');
footer.setAttribute("id", "ca_footer");

// var footnote = document.createElement('p');
// footnote.setAttribute("id", "ca_footnote");
// footnote.innerHTML = '*sample critiques are scored on a scale of 1-10, where 10 represents a great match (based on your preferences), and 1 represents a poor match (based on your preferences).';

var currOptions = document.createElement('p');
currOptions.setAttribute("id", "ca_curr_opts");
...
footer.appendChild(currOptions);
footer.appendChild(optionsLink);

caTooltipText.appendChild(header);
caTooltipText.appendChild(body);
caTooltipText.appendChild(footer);

sampleCrits.appendChild(caTooltipText);
spanToUse[0].appendChild(sampleCrits);
...
function getSentiment(phrase, inject, callback) {
    ...
    // Storage objects
    var tokens      = tokenizeString(phrase),
        score       = 0,
        words       = [],
        positive    = [],
        negative    = [];
...
/**
 * Returns an array of sum of input arrays
 */
...
/**
 * Ranks by length of critiques, by grouping them into three buckets
 * @param response - JSON response from get request
 * @param pref - user preference based on which critiques are sorted into buckets
...
function rankLength(response, pref) {
    ...
    for (var i = 0; i < numComments; i++) {
        var commentBody = response[1].data.children[i].data.body;
        if (pref == 1 && commentBody.length <= lim1) {
            lenArr.push(3);
        } else if (pref == 2 && (commentBody.length > lim1 && commentBody.length <= lim2)) {
            lenArr.push(3);
        } else if (pref == 3 && commentBody.length > lim2) {
            lenArr.push(3);
        } else {
            lenArr.push(0);
        }
...
/**
 * Ranks by sentiment of critiques by their sentiment scores
 * @param response - JSON response from get request
 * @param pref - user preference based on which critiques are sorted into buckets
...
function rankSentiment(response, pref) {
    ...
    for (var i = 0; i < numComments; i++) {
        var commentBody = response[1].data.children[i].data.body;
        var sntVal = getSentiment(commentBody);
        if (pref == 1 && sntVal.score < 0) {
            sntArr.push(Math.min(3, Math.abs(sntVal.score)));
        } else if (pref == 2 && sntVal.score == 0) {
            sntArr.push(Math.min(3, Math.abs(sntVal.score)));
        } else if (pref == 3 && sntVal.score > 0) {
            sntArr.push(Math.min(4, Math.abs(sntVal.score)));
        } else {
            sntArr.push(0);
        }
    }
...
/**
 * Ranks by usefulness of critiques based on the Reddit scores
 * @param response - JSON response from get request
 * @param pref - user preference based on which critiques are sorted into buckets
 * @returns {Array} - array of ranking for all the critiques in a post
 */
function rankActiveness(response, pref) {
    ...
    var numComments = response[1].data.children.length;
    for (var i = 0; i < numComments; i++) {
        var commentScore = response[1].data.children[i].data.ups;
        if (pref == 1 && commentScore < 0) {
            actArr.push(Math.min(3, Math.abs(commentScore)));
        } else if (pref == 2 && commentScore  == 0) {
            actArr.push(Math.min(3, Math.abs(commentScore)));
        } else if (pref == 3 && commentScore > 0) {
            actArr.push(Math.min(3, Math.abs(commentScore)));
        } else {
            actArr.push(0);
        }
...
/**
 * Gets a more user-friendly semantic of the current user preferences
 * @param pref - current user preference
 * @param kind - preference criteria
 * @returns {*} - string that is more semantic
 */
function getSemantic(pref, kind) {
    if (kind == 1) {
        if (pref == 1) {
            return "succinct, "
        } else if (pref == 2) {
            return "of average length, "
        } else if (pref == 3) {
            return "verbose, "
        }
    } else if (kind == 2) {
        if (pref == 1) {
            return "of negative sentiment, and "
        } else if (pref == 2) {
            return "of neutral sentiment, and "
        } else if (pref == 3) {
            return "of positive sentiment, and "
        }
    } else if (kind == 3) {
        if (pref == 1) {
            return "not that useful."
        } else if (pref == 2) {
            return "useful."
        } else if (pref == 3) {
            return "very useful."
...
/**
 * Gets a user-friendly semantic for the critique scores
 * @param score - critique score
 * @returns {*} - string that is more semantic
 */
function getScoreSemantic(score) {
    if (score >= 8) {
    ...
    getJSON(searchURL,
        function (error, responseData) {
            if (error !== null || responseData === null || responseData === undefined) {
                ...
                var i = getRandomInt(0, 24);
                var j = getRandomInt(0, 24);
                while (i === j) {
                    j = getRandomInt(0, 24);
                }

                var post1 = responseData.data.children[i].data.url;
                post1 = post1 + ".json?sort=confidence";

                var post2 = responseData.data.children[j].data.url;
                post2 = post2 + ".json?sort=confidence";
                ...
                getJSON(post1,
                    function (exError, exResponseData) {
                        if (exError !== null) {
                            ...
                            var lenArr = rankLength(exResponseData, userPrefs.valLen);
                            var sntArr = rankSentiment(exResponseData, userPrefs.valSnt);
                            var spcArr = rankActiveness(exResponseData, userPrefs.valSpc);

                            var sumArr = arraySum(lenArr, sntArr);
                            var finalSum = arraySum(sumArr, spcArr);

                            var x = Math.max.apply(Math, finalSum);
                            var idx = finalSum.indexOf(x);

                            document.getElementById("crit_col1_data").innerHTML = exResponseData[1].data.children[idx].data.body;
                            document.getElementById("crit_col1_head").innerHTML = "Here's a critique that is " + getScoreSemantic(x) + " with your preferences*:";
                            document.getElementById("ca_crit1_link").href = baseURL + exResponseData[1].data.children[idx].data.permalink;
                        }
                    });
                ...
                getJSON(post2,
                    function (exError, exResponseData) {
                        if (exError !== null) {
                            ...
                            var x = Math.min.apply(Math, finalSum);
                            var idx = finalSum.indexOf(x);

                            document.getElementById("crit_col2_data").innerHTML = exResponseData[1].data.children[idx].data.body;
                            document.getElementById("crit_col2_head").innerHTML = "Here's a critique that is " + getScoreSemantic(x) + " with your preferences*:";
                            document.getElementById("ca_crit2_link").href = baseURL + exResponseData[1].data.children[idx].data.permalink;
                        }
                    });
            }
        });
